...
def encode_generic(text):
    """Returns encoded string code with format [encoded_huffman_tree][extra_zeros_num][encoded_text]"""
                                       # Ordenar els caràcters en ordre de més a menys freqüència
    code_table = {}
    _fill_code_table(huffman_tree, "", code_table)                  # Afegeix la informació de la LUT al codi que s'envia per descomprimir
...
def compress_specific(input_path, output_path):
    ...
    with open(input_path) as in_file, open(output_path, "wb") as out_file:
        ...
        encoded_text = encode_specific(text)                                     # Es comprimeix amb Huffman
...
def compress_generic(input_path, output_path):
    ...
    with open(input_path) as in_file, open(output_path, "wb") as out_file:
        text = in_file.read()                                           # Es llegeix el fitxer
        encoded_text = encode_generic(text)                                     # Es comprimeix amb Huffman
...
if __name__ == "__main__":
    file_to_compress, compressed = "compress-me-specific.txt", "compressed_specific.txt"
    compress_specific(file_to_compress, compressed)
    file_to_compress, compressed = "compress-me-generic.txt", "compressed_generic.txt"
    compress_generic(file_to_compress, compressed)
